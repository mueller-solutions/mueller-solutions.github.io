...
const handleScroll = debounce(() => {
  ...
  navLinkEls.forEach((navLinkEl) => {
    ...
    if (
      currentSection === 'home' &&
      navLinkEl.href === window.location.origin + window.location.pathname + window.location.search
    ) {
      navLinkEl.classList.add('active');
    }
  });
...
navLinkEls.forEach((navLinkEl) => {
  navLinkEl.addEventListener('click', (event) => {
    ...
    setTimeout(() => {
      ...
      navEl.setAttribute('style', `--height: 0`);
    }, 100);
    ...
    if (navLinkEl.href === window.location.origin + window.location.pathname + window.location.search) {
    ...
    history.pushState('', document.title, window.location.pathname + '#/' + targetId + window.location.search);
...
navToggleEl.addEventListener('click', () => {
  ...
  if (visibility === 'false') {
    ...
    navEl.setAttribute('style', `--height: ${navListHeight}px`);
  } else {
    navToggleEl.setAttribute('aria-expanded', 'false');
    navEl.setAttribute('style', `--height: 0`);
  }
